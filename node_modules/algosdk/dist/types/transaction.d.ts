...
import AnyTransaction, { EncodedTransaction, EncodedMultisig, EncodedLogicSig, EncodedHeartbeatFields, EncodedHeartbeatProof } from './types/transactions';
import { Address } from './types/address';
export declare const ALGORAND_MIN_TX_FEE = 1000;
/**
 * Object representing a heartbeat proof
 */
declare class HeartbeatProof {
    s: Buffer;
    p: Buffer;
    p2: Buffer;
    p1s: Buffer;
    ...
    static from_obj_for_encoding(encHbFields: EncodedHeartbeatProof): HeartbeatProof;
    get_obj_for_encoding(): EncodedHeartbeatProof;
}
/**
 * Object representing a heartbeat transaction
 */
declare class HeartbeatFields {
    a: string | Address;
    prf: HeartbeatProof;
    sd: Buffer;
    vid: Buffer;
    kd: number;
    constructor(addr: string | Address, proof: EncodedHeartbeatProof, seed: Buffer, voteID: Buffer, kd: number);
    static from_obj_for_encoding(encHbFields: EncodedHeartbeatFields): HeartbeatFields;
    get_obj_for_encoding(): EncodedHeartbeatFields;
}
/**
 ...
 */
interface TransactionStorageStructure extends Omit<TransactionParams, 'from' | 'to' | 'genesisHash' | 'closeRemainderTo' | 'voteKey' | 'selectionKey' | 'stateProofKey' | 'assetManager' | 'assetReserve' | 'assetFreeze' | 'assetClawback' | 'assetRevocationTarget' | 'freezeAccount' | 'appAccounts' | 'suggestedParams' | 'reKeyTo' | 'heartbeatFields'> {
    from: string | Address;
    to: string | Address;
    fee: number;
